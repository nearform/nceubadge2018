...
Badge.scan = on => {
  ...
  if (on) Badge.scanInterval = setInterval(() => Badge.scanOnce(1), interval);
  Badge.scanOnce(on);
};
Badge.scanOnce = on => {
  ...
  var WL = [
    ...
    "b8:27:eb:12:b7:71 public",
    "b8:27:eb:b9:1d:f2 public"
  ];
...
`);
  ...
  function draw(n) {
    ...
    function addTalkInfo(m1, m2) {
      var msg1 = Badge.bleData[m1],
        msg2 = Badge.bleData[m2];
      if (msg1) {
...
Badge.apps["Privacy"] = firstRun => {
  ...
  menu[
    "Send Anon. Location : " + (Badge.settings.location ? "Yes" : "No")
  ] = toggle("location");
  menu[
    "Get Alerts/Info : " + (Badge.settings.allowScan ? "Yes" : "No")
  ] = toggle("allowScan");
...
function loadSettings() {
  ...
  try {
    ...
    for (var i in o) if ("boolean" == typeof o[i]) Badge.settings[i] = o[i];
  } catch (e) {
